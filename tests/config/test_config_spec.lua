...
describe(" Config Methods Tests ", function()
	...
	after_each(function()
		lib.luagit2_config_free(Config_ondisk)
	...
	it("Test Setting and Getting Config Values",function()
		...
		Config_Snapshot = lib.luagit2_repository_config_snapshot(repo)
		...
		local cfg_int32 = lib.luagit2_config_get_int32(Config_Snapshot,"core.repositoryformatversion")
		local cfg_int64 = lib.luagit2_config_get_int64(Config_Snapshot,"core.repositoryformatversion")
		local cfg_bool = lib.luagit2_config_get_bool(Config_Snapshot,"core.filemode")

		local cfg_path_buf = lib.luagit2_config_get_path(Config_Snapshot,"remote.origin.url")
		assert.are.same("https://github.com/libgit2/false.git",lib.luagit2_buf_details(cfg_path_buf))

		local cfg_str_buf = lib.luagit2_config_get_string_buf(Config_Snapshot,"user.name")
		assert.are.same("TEST_USER",lib.luagit2_buf_details(cfg_str_buf))
		...
		lib.luagit2_config_set_string(Config_ondisk,"user.email","newemail@example.com")
		lib.luagit2_config_set_string(Config_ondisk,"user.name","NEW_NAME_TEST_USER")
		lib.luagit2_config_set_int32(Config_ondisk,"core.repositoryformatversion",10)
		lib.luagit2_config_set_int64(Config_ondisk,"core.repositoryformatversion",10)
		lib.luagit2_config_set_bool(Config_ondisk,"core.filemode",0)
		...
		local new_int32 = lib.luagit2_config_get_int32(Config_Snapshot,"core.repositoryformatversion")
		local new_bool = lib.luagit2_config_get_bool(Config_ondisk,"core.filemode")
	...
	it("Tests config open default ", function()
		Config_Snapshot = lib.luagit2_config_snapshot(Config_Default)
		local Def_name = lib.luagit2_config_get_string(Config_Snapshot,"user.name")
		local Def_email = lib.luagit2_config_get_string(Config_Snapshot,"user.email")
	...
	it("Tests find System, Global Config path ", function()
		...
		local find_sys = lib.luagit2_config_find_system()
		
		-- Current Tests are run using a non-Windows system
		-- Thus these Values can't be found
		-- 
		-- local find_programdata = lib.luagit2_config_find_programdata()
		-- local fing_xdg = lib.luagit2_config_find_xdg()

		assert.are.equal("/etc/gitconfig",lib.luagit2_buf_details(find_sys))

		-- In Unix System, Path for git global id ~/.gitconfig
		assert.are.equal('',lib.luagit2_buf_details(find_global))
	end)
	...
	it("Tests various Config get methods ",function()
		...
		cfg_bool = lib.luagit2_config_get_bool(Config_Snapshot,"core.bare")
	...
	it("Tests Config open level ",function()
		...
		--
		----------------------------------------------------------------------------------
		local cfg_lvl_sys = lib.luagit2_config_level_system()
		local sys_lvl_cfg = lib.luagit2_config_open_level(Config_Default,cfg_lvl_sys)
		Config_Snapshot = lib.luagit2_config_snapshot(sys_lvl_cfg)
		...
		--
		-----------------------------------------------------------------------------------
		local cfg_lvl_global = lib.luagit2_config_level_global()
		local global_lvl_cfg = lib.luagit2_config_open_level(Config_Default,cfg_lvl_global)
		Config_Snapshot = lib.luagit2_config_snapshot(global_lvl_cfg)
		...
		-- 
		-- 
		local cfg_lvl_local = lib.luagit2_config_level_local()
		local local_lvl_cfg = lib.luagit2_config_open_level(Config_ondisk,cfg_lvl_local)
		Config_Snapshot = lib.luagit2_config_snapshot(local_lvl_cfg)
		...
		--
		------------------------------------------------------------------------------------
		local cfg_lvl_program_data = lib.luagit2_config_level_program_data()
		local cfg_lvl_xdg = lib.luagit2_config_level_xdg()
	end)

	it("Tests Varioud Parse Methods",function()
		local parse_bool = lib.luagit2_config_parse_bool("true")
		assert.are.equal(1,parse_bool)
		parse_bool = lib.luagit2_config_parse_bool("yes")
		assert.are.equal(1,parse_bool)
		parse_bool = lib.luagit2_config_parse_bool("on")
		assert.are.equal(1,parse_bool)

		local parse_int32 = lib.luagit2_config_parse_int64("32")
		assert.are.equal(32,parse_int32)

		local parse_int64 = lib.luagit2_config_parse_int64("999988892")
		assert.are.equal(999988892,parse_int64)

		local parse_path = lib.luagit2_config_parse_path("abc/def/ghi")
		assert.are.equal("abc/def/ghi",lib.luagit2_buf_details(parse_path))
	end)
	...
	it("Tests Deleting Config Variables",function()
		...
		-- Delete a variable
		--
		lib.luagit2_config_delete_entry(Config_ondisk,"user.name")
		lib.luagit2_config_delete_entry(Config_ondisk,"user.email")
	...
	it("Tests adding a new Config on disk",function()
		...
		--
		local new_cfg = lib.luagit2_config_new()
		-- Add the config to an EXISTING FILE
		local cfg_lvl_local = lib.luagit2_config_level_local()
		lib.luagit2_config_add_file_ondisk(new_cfg,"Fixtures/WORKON_REPO/abc.txt",
										cfg_lvl_local,repo,1)
		-- Set a variable
		lib.luagit2_config_set_string(new_cfg,"user.name","New_config_user")
		-- Get a snapshot 
		Config_Snapshot = lib.luagit2_config_snapshot(new_cfg)
		-- Check for the variable
		local name = lib.luagit2_config_get_string(Config_Snapshot,"user.name")
		assert.are.same("New_config_user",name)
		-- Free used Config's
		lib.luagit2_config_free(new_cfg)
		lib.luagit2_config_free(Config_Snapshot)
		...
		--
		local new_cfg_ondisk =  lib.luagit2_config_open_ondisk(current_directory_path .. "/Fixtures/WORKON_REPO/abc.txt")
		Config_Snapshot = lib.luagit2_config_snapshot(new_cfg_ondisk)
		-- Check for the variable
		local name = lib.luagit2_config_get_string(Config_Snapshot,"user.name")
		assert.are.same("New_config_user",name)
		-- Free used Config's
		lib.luagit2_config_free(new_cfg_ondisk)
		lib.luagit2_config_free(Config_Snapshot)
	end)

	it("Tests config next",function()
		local cfg_iterator = lib.luagit2_config_iterator_new(Config_ondisk)
		local cfg_entry = lib.luagit2_config_next(cfg_iterator)

		local first_entry_name = lib.luagit2_config_entry_name(cfg_entry)
		-- Check that this entry exists
		assert.is_not_nil(lib.luagit2_config_get_string(Config_ondisk,first_entry_name))

		local second_entry_name = lib.luagit2_config_entry_name(cfg_entry)
		-- Check that this entry exists
		assert.is_not_nil(lib.luagit2_config_get_string(Config_ondisk,second_entry_name))

		-- Free up used objects
		lib.luagit2_config_iterator_free(cfg_iterator)
	end)

	it("Tests config get entry",function()
		-- Open a  config entry
		local cfg_entry = lib.luagit2_config_get_entry(Config_ondisk,"user.name")
		-- Check the value
		assert.are.equal("TEST_USER",lib.luagit2_config_entry_value(cfg_entry))
