...
describe(" Repository Methods Tests ", function()
	...
	local luagit2_repository = require("repository.repository_cover")
	local repo
	...
	before_each(function()
		...
		repo = luagit2_repository.repository_open("Fixtures/WORKON_REPO/.git")
	end)

	after_each(function()
		luagit2_repository.repository_free(repo)
		fixer.set_back()
	end)
	...
	it("Tests Repository Commondir",function()
		...
		local common_dir = luagit2_repository.repository_commondir(repo)
	...
	it("Tests Repository Config & Repository Config Snapshot",function()
		...
		local Config = luagit2_repository.repository_config(repo)
		-- Create a Snapshot to read the existing values 
		local Config_Snapshot = luagit2_repository.repository_config_snapshot(repo)
	...
	it("Tests detach head",function()
		...
		local is_head_detached = luagit2_repository.repository_head_detached(repo)
		assert.are.equal(false,is_head_detached)

		-- Detach the HEAD for the test repo
		luagit2_repository.repository_detach_head(repo)

		-- Check again if HEAD is detached
		is_head_detached = luagit2_repository.repository_head_detached(repo)
	...
	it("Tests repository namespace",function()
		...
		local namespace = luagit2_repository.repository_get_namespace(repo)
		assert.is_nil(namespace)

		-- Lets Set the namespace for repo and check again
		luagit2_repository.repository_set_namespace(repo,"SOME_EXAMPLE_NAMESPACE")
		local namespace_new = luagit2_repository.repository_get_namespace(repo)
	...
	it("Tests getting Repository HEAD",function()
		...
		local repo_HEAD_ref = luagit2_repository.repository_head(repo)
	...
	it("Tests Head unborn",function()
		...
		local is_head_unborn = luagit2_repository.repository_head_unborn(repo)
		...
		local repo_init = luagit2_repository.repository_init("Fixtures/WORKON_REPO_INIT",0)
		-- Check if head is unborn or not
		local is_initialized_repo_head_unborn = luagit2_repository.repository_head_unborn(repo_init)
		assert.are.equal(true,is_initialized_repo_head_unborn)

		-- Free used repo && remove the files
		luagit2_repository.repository_free(repo_init)
		os.execute('cd Fixtures && rm -rf WORKON_REPO_INIT')
	end)

	it("Tests Repository index", function()
		-- Get the index for the current repo
		local index = luagit2_repository.repository_index(repo)
		...
		local owner_repo_path = luagit2_repository.repository_path(owner_repo)
	...
	it("Tests Repository identity",function()
		...
		-- get nil values.
		local old_name, old_email  = luagit2_repository.repository_ident(repo)
		assert.is_nil(old_name)
		assert.is_nil(old_email)


		-- Lets set the identity values & check again
		luagit2_repository.repository_set_ident(repo,"EXAMPLE_NAME","something@something.com")
		local new_name,new_email  = luagit2_repository.repository_ident(repo)
	...
	it("Tests repository bare methods",function()
		...
		local is_bare = luagit2_repository.repository_is_bare(repo)
		assert.are.equal(false,is_bare)

		-- set the repository to bare
		luagit2_repository.repository_set_bare(repo)
		-- Check again
		is_bare = luagit2_repository.repository_is_bare(repo)
	...
	it("Tests check-repository is_empty, is_shallow, is_worktree methods",function()
		...
		local is_empty = luagit2_repository.repository_is_empty(repo)
		assert.are.equal(false,is_empty)

		local is_shallow = luagit2_repository.repository_is_shallow(repo)
		assert.are.equal(false,is_shallow)
		
		local is_worktree = luagit2_repository.repository_is_worktree(repo)
		assert.are.equal(false,is_worktree)
	end)

	it("Tests repositoy odb",function()
		-- improve this test later
		local repo_odb = luagit2_repository.repository_odb(repo)
		assert.is_not_nil(repo_odb)
	end)

	it("Tests open repository as bare",function()
		-- open a repo in bare form 
		local bare_repo = luagit2_repository.repository_open_bare("Fixtures/WORKON_REPO/.git")
		-- Check if the opened repo is bare or not
		local repo_is_bare = luagit2_repository.repository_is_bare(bare_repo)
		assert.are.equal(true,repo_is_bare)

		-- Free used repo
		luagit2_repository.repository_free(bare_repo)
	end)
	...
	it("Tests set repository Head",function()
		...
		luagit2_repository.repository_set_head(repo,"refs/heads/executable")
		local repo_HEAD_ref = luagit2_repository.repository_head(repo)
		local ref_name = luagit2.reference_name(repo_HEAD_ref)
	...
	it("Tests repository submodule related methods",function()
		luagit2_repository.repository_submodule_cache_all(repo)
		luagit2_repository.repository_submodule_cache_clear(repo)
	end)

	it("Tests repo cleanup",function()
		luagit2_repository.repository_cleanup(repo)
	end)
end)
