...
describe(" Reference Methods Tests ", function()
	...
	local luagit2_reference = require("reference.reference_cover")
	local repo
	...
	before_each(function()
		...
		ref_dir = luagit2_reference.reference_lookup(repo,"refs/heads/dir")
		ref_point_to_blob = luagit2_reference.reference_lookup(repo,"refs/tags/point_to_blob")
	end)

	after_each(function()
		luagit2_reference.reference_free(ref_dir)
		luagit2_reference.reference_free(ref_point_to_blob)
	...
	it("Checks the reference, if it is branch | note | remote | tag ",function()
		...
		local is_branch_A = luagit2_reference.reference_is_branch(ref_dir)
		local is_branch_B = luagit2_reference.reference_is_branch(ref_point_to_blob)

		local is_note_A = luagit2_reference.reference_is_note(ref_dir)
		local is_note_B = luagit2_reference.reference_is_note(ref_point_to_blob)

		local is_remote_A = luagit2_reference.reference_is_remote(ref_dir)
		local is_remote_B = luagit2_reference.reference_is_remote(ref_point_to_blob)

		local is_tag_A = luagit2_reference.reference_is_tag(ref_dir)
		local is_tag_B = luagit2_reference.reference_is_tag(ref_point_to_blob)
	...
	it("Checks for a valid name",function()
		...
		local is_valid_name_A = luagit2_reference.reference_is_valid_name("refs/tags/packed-tag")
		local is_valid_name_B = luagit2_reference.reference_is_valid_name("refs/tags/alpha@{beta")
	...
	it("Checks for reference name and its oid",function()
		local ref_name = luagit2_reference.reference_name(ref_dir)
		assert.are.equal("refs/heads/dir",ref_name)

		local ref_oid = luagit2_reference.reference_name_to_id(repo,"refs/heads/dir")
	...
	it("Tests reference remove",function()
		...
		local ref_executable = luagit2_reference.reference_lookup(repo,"refs/heads/executable")
		local ref_name = luagit2_reference.reference_name(ref_executable)
		assert.are.equal("refs/heads/executable",ref_name)

		-- Lets remove that reference from repo.
		luagit2_reference.reference_remove(repo,"refs/heads/executable")
		...
		--
		-- try running this
		-- ref_executable = luagit2_reference.reference_lookup(repo,"refs/heads/executable"
	end)
	...
	it("Tests reference rename ",function()
		...
		-- rename the ref
		luagit2_reference.reference_rename(ref_executable,"refs/heads/new_name_for_executable",1,"changed name refs/heads/executable")

		-- Lets lookup for the reference with changed name.
		ref_executable = luagit2_reference.reference_lookup(repo,"refs/heads/new_name_for_executable")
		ref_name = luagit2_reference.reference_name(ref_executable)
	...
	it("Tests reference target related functions",function()
		...
		local target_id = luagit2_reference.reference_target(ref_dir)
		...
		local new_target_ref = luagit2_reference.reference_set_target(ref_dir,new_target_oid,"Reset target of ref_dir")

		-- check for the target the reference points at.
		local target_id_check = luagit2_reference.reference_target(new_target_ref)
		...
		local ref_name = luagit2_reference.reference_name(new_target_ref)
		assert.are.equal("refs/heads/dir",ref_name)
	...
	it("Tests reference iterator related functions",function()
		...
		local ref_iterator = luagit2_reference.reference_iterator_new(repo)

		-- Get the next reference and check its name
		local next_ref_first = luagit2_reference.reference_next(ref_iterator)
		local name = luagit2_reference.reference_name(next_ref_first)
		assert.are.equal("refs/heads/br2",name)

		local next_ref_second = luagit2_reference.reference_next_name(ref_iterator)
	...
	it("Tests reference Create",function()
		...
		local ref_create = luagit2_reference.reference_create(repo,"refs/heads/a_new_ref",target_oid,1,"created new reference")

		-- Check the name and target of created reference.
		local ref_name = luagit2_reference.reference_name(ref_create)
		assert.are.equal("refs/heads/a_new_ref",ref_name)

		local target_id = luagit2_reference.reference_target(ref_create)
		local target_id_str = luagit2.oid_tostr_s(target_id)
		assert.are.equal(ref_dir_str,target_id_str)

		-- Free the used reference
		luagit2_reference.reference_free(ref_create)
	end)
	...
	it("Tests reference Create matching",function()
		...
		local target_oid = luagit2.oid_fromstr(ref_dir_str)
		local matching_reference_oid = luagit2_reference.reference_name_to_id(repo,"refs/tags/point_to_blob")

		local ref_create_matching = luagit2_reference.reference_create_matching(repo,"refs/tags/point_to_blob",target_oid,1,
			matching_reference_oid,"created new reference")

		-- Check the name and target of created reference.
		local ref_name = luagit2_reference.reference_name(ref_create_matching)
		assert.are.equal("refs/tags/point_to_blob",ref_name)

		local target_id = luagit2_reference.reference_target(ref_create_matching)
		local target_id_str = luagit2.oid_tostr_s(target_id)
		assert.are.equal(ref_dir_str,target_id_str)

		luagit2_reference.reference_free(ref_create_matching)
	end)

	it("Tests reference symbolic Create",function()
		-- Create a new reference that
		-- targets to "ref/heads/dir"
		local ref_create_sym = luagit2_reference.reference_symbolic_create(repo,"refs/heads/a_new_ref","ref/heads/dir",
			1,"created new reference")

		-- Check the name of created reference.
		local ref_name = luagit2_reference.reference_name(ref_create_sym)
		assert.are.equal("refs/heads/a_new_ref",ref_name)

		luagit2_reference.reference_free(ref_create_sym)
	end)
