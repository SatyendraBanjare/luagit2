...
describe(" Signature Methods Tests ", function()
    ...
    local luagit2_signature = require("signature.signature_cover")
    ...
    it("Tests signature_default method", function()
        ...
        local sign_default = luagit2_signature.signature_default(repo)
        ...
        luagit2_signature.signature_free(sign_default)
    end)
    ...
    it("Tests signature from string method", function()
        ...
        local sign_from_string_buffer = luagit2_signature.signature_from_buffer(sign_string)
        ...
        -- free the signature
        luagit2_signature.signature_free(sign_from_string_buffer)
    end)
    
    it("Tests signature now method", function()
        local sign_now = luagit2_signature.signature_now("new_user", "new_user@example.com")
        ...
        -- free the signature
        luagit2_signature.signature_free(sign_now)
    end)
    
    it("Tests signature dup method", function()
        local sign_now = luagit2_signature.signature_now("new_user", "new_user@example.com")
        local sign_dup = luagit2_signature.signature_dup(sign_now)
        ...
        -- free the signatures
        luagit2_signature.signature_free(sign_now)
        luagit2_signature.signature_free(sign_dup)
    end)
end)
