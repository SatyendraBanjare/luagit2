...
describe(" Blob Methods Tests ", function()
	...
	local luagit2_blob = require("blob.blob_cover")
	...
	before_each(function()
		...
		README_blob_oid = luagit2_blob.blob_create_fromdisk(repo,repo_path .. "/README")
		abc_txt_blob_oid = luagit2_blob.blob_create_fromdisk(repo,repo_path .. "/abc.txt")
		README_blob = luagit2_blob.blob_lookup(repo,README_blob_oid)
		abc_txt_blob = luagit2_blob.blob_lookup(repo,abc_txt_blob_oid)
	end)

	after_each(function()
		luagit2_blob.blob_free(README_blob)
		luagit2_blob.blob_free(abc_txt_blob)
	...
	it("Tests loking up blob in the repo",function()
		...
		local README_id = luagit2_blob.blob_id(README_blob)
		assert.are.equal(README_oid_string,luagit2.oid_tostr_s(README_id))

		local blob_id = luagit2_blob.blob_id(abc_txt_blob)
	...
	it("Tests Creating Blob oid from workdir",function()
		...
		-- used git repository 
		local workdir_blob_id = luagit2_blob.blob_create_fromworkdir(repo,"README")
	...
	it("Tests Blob lookup prefix",function()
		...
		-- n lengthed prefix
		local readme_lookup_prefix = luagit2_blob.blob_lookup_prefix(repo,README_blob_oid,5)
		local readme_lookup_prefix_oid = luagit2_blob.blob_id(readme_lookup_prefix)
		assert.are.equal(README_oid_string,luagit2.oid_tostr_s(readme_lookup_prefix_oid))
		luagit2_blob.blob_free(readme_lookup_prefix)
	end)

	it("Tests Blob Owner Repository",function()
		-- Get the owner repository
		local owner_repo = luagit2_blob.blob_owner(README_blob)
	...
	it("Tests if Blob is binary",function()
		...
		assert.are.equal(false,luagit2_blob.blob_is_binary(README_blob))
	end)
