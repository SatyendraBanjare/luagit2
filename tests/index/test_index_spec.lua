...
describe(" Index Methods Tests ", function()
    ...
    it("Tests index version methods", function()
        local index_version = luagit2.index_version(repo_index)
        assert.is.number(index_version)

        luagit2.index_set_version(repo_index, 4)

        local index_version_new = luagit2.index_version(repo_index)
    ...
    it("Tests git index find", function()
        ...
        local index_entry_position = luagit2.index_find(repo_index, "abc.txt")
        assert.are.equal(0, index_entry_position)

        -- Similarily Lets check for another file `new.txt`
        -- its index position must be 1.
        local index_entry_position_2 = luagit2.index_find(repo_index, "new.txt")
        ...
        local index_entry = luagit2.index_get_byindex(repo_index, index_entry_position)
        local entry_path = luagit2.index_entry_get_path(index_entry)
        assert.are.equal("abc.txt", entry_path)

        local index_entry_2 = luagit2.index_get_byindex(repo_index, index_entry_position_2)
        local entry_path_2 = luagit2.index_entry_get_path(index_entry_2)
        assert.are.equal("new.txt", entry_path_2)
    end)

    it("Tests index checksum", function()
        -- Calculates checksum for the index
        local index_oid = luagit2.index_checksum(repo_index)
    ...
    it("Tests index caps related methods", function()
        local index_caps = luagit2.index_caps(repo_index)
        assert.are.equal(1, index_caps)

        -- set the caps value and check again
        luagit2.index_set_caps(repo_index, 0)
        local index_caps_new = luagit2.index_caps(repo_index)
    ...
    it("Tests index add related methods", function()
        ...
        --
        -- Add to index
        luagit2.index_add_bypath(repo_index, "something.txt")

        -- Next lets try to find the recently added file in repo.
        local index_entry_position = luagit2.index_find(repo_index, "something.txt")
    ...
    it("Tests Removing files and directory from index", function()
        -- Lets remove a file
        luagit2.index_remove_bypath(repo_index, "new.txt")

        -- Try to get the file from index
        local index_val = luagit2.index_find(repo_index, "new.txt")

        local index_entry = luagit2.index_get_byindex(repo_index, index_val)
        ...
        -- Expected
        assert.has_error(function() luagit2.index_entry_get_path(index_entry) end)
        ...
        index_val = luagit2.index_find(repo_index, "subdir/new.txt")
        index_entry = luagit2.index_get_byindex(repo_index, index_val)
        local stage_val = luagit2.index_entry_stage(index_entry)

        -- Remove the directory
        luagit2.index_remove_directory(repo_index, "subdir", stage_val)
    ...
    it("Tests index write tree", function()
        ...
        local oid = luagit2.index_write_tree(repo_index)
    ...
    it("Tests index write tree to repo", function()
        -- get the tree oid for the used index
        local oid = luagit2.index_write_tree_to(repo_index, repo)
