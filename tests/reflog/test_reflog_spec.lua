...
describe(" Reflog Methods Tests ", function()
    ...
    before_each(function()
        ...
        head_reflog = luagit2.reflog_read(repo, "HEAD")
        master_reflog = luagit2.reflog_read(repo, "master")
    end)
    
    after_each(function()
        luagit2.reflog_free(head_reflog)
        luagit2.reflog_free(master_reflog)
    ...
    it("tests reflog entry count", function()
        local count = luagit2.reflog_entrycount(head_reflog)
    ...
    it("tests reflog entry by index", function()
        ...
        local reflog_entry = luagit2.reflog_entry_byindex(head_reflog, 0)
    ...
    it("tests reflog entry drop", function()
        local count_old = luagit2.reflog_entrycount(head_reflog)
        assert.are.same(2, count_old)
        
        -- drop a reflog entry
        luagit2.reflog_drop(head_reflog, 0, 0) -- do not rewrite
        
        -- check if count has reduced or not
        local count_new = luagit2.reflog_entrycount(head_reflog)
        assert.are.same(1, count_new)
    end)
    
    it("tests reflog delete", function()
        assert.has_no_errors(function() luagit2.reflog_delete(repo, "HEAD@{1}") end)
    end)
    ...
    it("tests reflog append", function()
        ...
        luagit2.reflog_append(head_reflog, commit_oid, sign_now, "Some message")
