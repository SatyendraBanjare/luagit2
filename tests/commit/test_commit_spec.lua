...
describe(" Commit Methods Tests ", function()
	...
	before_each(function()
		...
		commit_a = luagit2.commit_lookup(repo,commit_id_a)
		commit_b = luagit2.commit_lookup(repo,commit_id_b)
		author_sign_a = luagit2.commit_author(commit_a)
		author_sign_b = luagit2.commit_author(commit_b)
		committer_sign_a = luagit2.commit_committer(commit_a)
		committer_sign_b = luagit2.commit_committer(commit_b)
	end)

	after_each(function()
		luagit2.commit_free(commit_a)
		luagit2.commit_free(commit_b)
	...
	it("Tests commit Body",function()
		assert.are.equal("Commit message body",luagit2.commit_body(commit_a))
		assert.is_nil(luagit2.commit_body(commit_b))
	end)
	...
	it("Tests extract commit Header Field ", function()
		...
		-- Should be received
		local parent_buf = luagit2.commit_header_field(commit_a,"parent")
		assert.are.equal(commit_id_b_string, luagit2.buf_details(parent_buf))

		-- We should get the current write-tree id
		local tree_buf = luagit2.commit_header_field(commit_a,"tree")
	...
	it("Tests commit lookup using prefix", function()
		...
		local Commit_test_id = luagit2.commit_id(Commit_test)
		assert.are.equal(commit_id_a_string, luagit2.oid_tostr_s(Commit_test_id))
		luagit2.commit_free(Commit_test)
	end)

	it("Tests commit Message",function()
		local message = luagit2.commit_message(commit_b)
	...
	it("Tests commit Message encoding ",function()
		...
		local message_encoding = luagit2.commit_message_encoding(commit_a)
		assert.are.same(nil,message)
	end)

	it("Tests Commit Message Raw", function()
		local message_raw = luagit2.commit_message(commit_b)
		assert.are.same("subdirectories\n",message_raw)
	end)

	it("Tests Commit Nth generation parent",function()
		-- Zero generation ancestor means the commit itself
		local commit_gen_zero = luagit2.commit_nth_gen_ancestor(commit_a,0)
		-- Check if we got the same commit or not
		local commit_gen_zero_id = luagit2.commit_id(commit_gen_zero)
		assert.are.equal(commit_id_a_string, luagit2.oid_tostr_s(commit_gen_zero_id))

		-- generation one means immediate parent commit
		local commit_gen_one = luagit2.commit_nth_gen_ancestor(commit_a,1)
		-- Check for values
		local commit_gen_one_id = luagit2.commit_id(commit_gen_one)
		assert.are.equal(commit_id_b_string, luagit2.oid_tostr_s(commit_gen_one_id))
		luagit2.commit_free(commit_gen_zero)
		luagit2.commit_free(commit_gen_one)
	end)
	...
	it("Tests Commit Parent",function()
		...
		local parent_commit_one = luagit2.commit_parent(commit_a,0)

		local parent_commit_id = luagit2.commit_id(parent_commit_one)
		assert.are.equal(commit_id_b_string, luagit2.oid_tostr_s(parent_commit_id))
		luagit2.commit_free(parent_commit_one)
	end)
	...
	it("Tests commit Parent ID",function()
		...
		-- the param value of 0 means we want 
		-- just immediate before value of commit.
		local parent_commit_id = luagit2.commit_parent_id(commit_a,0)
		assert.are.equal(commit_id_b_string, luagit2.oid_tostr_s(parent_commit_id))
	end)
	...
	it("Tests parent count",function()
		...
		-- its parent count 
		local parent_count = luagit2.commit_parentcount(commit_a)
	...
	it("Tests Raw Commit Header",function()
		...
		local Header_A = luagit2.commit_raw_header(commit_a)
		assert.are.equal(Header,Header_A)
	end)

	it("Tests Commit Summary", function()
		local commit_a_summary = luagit2.commit_summary(commit_a)
		assert.are.equal("Added File",commit_a_summary)
	end)

	it("Tests Commit Tree", function()
		local commit_a_tree = luagit2.commit_tree(commit_a)
	...
	it("Tests Commit Tree Id",function()
		...
		local tree_id = luagit2.commit_tree_id(commit_a)
		assert.are.equal(current_write_tree_id, luagit2.oid_tostr_s(tree_id))
	end)
	...
	it("Tests Creating a Commmit and update head ",function ()
		...
		local new_commit_id =  luagit2.commit_create_update_head(repo,author_sign_a,
		...
		local new_commit = luagit2.commit_lookup(repo,new_commit_id)
		-- Check the lookedup Commit is the one that we created
		local lookedup_commit_id = luagit2.commit_id(new_commit)
	...
	it("Tests Creating a Commmit and update none ",function ()
		...
		-- Create a new Commit and get commit Id
		local new_commit_id =  luagit2.commit_create_update_none(repo,author_sign_a,
			committer_sign_a,"New Commit From luagit2",parent_tree,commit_a)
	...
	it(" Tests create a signed commit and extract commit signature ", function()
		...
		local sign_buf, extra_buf = luagit2.commit_extract_signature(repo,new_commit_id)

		assert.are.equal(gpgsig,luagit2.buf_details(sign_buf))
	end)

	it("Tests commit time",function()
		local time_str = luagit2.commit_time(commit_a)
