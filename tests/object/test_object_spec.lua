...
describe(" Object Methods Tests ", function()
	...
	before_each(function()
		...
		otype_blob = luagit2.object_string2type("blob")
		otype_tree = luagit2.object_string2type("tree")
		otype_tag = luagit2.object_string2type("tag")
		otype_commit = luagit2.object_string2type("commit")
		obj_blob = luagit2.object_lookup(repo,blob_id,otype_blob)
		obj_tree = luagit2.object_lookup(repo,tree_id,otype_tree)
		obj_tag = luagit2.object_lookup(repo,tag_id,otype_tag)
		obj_commit = luagit2.object_lookup(repo,commit_id,otype_commit)
	end)

	after_each(function()
		luagit2.object_free(obj_blob)
		luagit2.object_free(obj_tree)
		luagit2.object_free(obj_tag)
		luagit2.object_free(obj_commit)
	...
	it("Tests getting object ID ",function()
		local check_blob_id = luagit2.object_id(obj_blob)
		local check_blob_id_str = luagit2.oid_tostr_s(check_blob_id)

		local check_tag_id = luagit2.object_id(obj_tag)
		local check_tag_id_str = luagit2.oid_tostr_s(check_tag_id)

		local check_tree_id = luagit2.object_id(obj_tree)
		local check_tree_id_str = luagit2.oid_tostr_s(check_tree_id)

		local check_commit_id = luagit2.object_id(obj_commit)
	...
	it("Checks Object Type ",function()
		local type_blob = luagit2.object_type(obj_blob)
		local type_tree = luagit2.object_type(obj_tree)
		local type_tag = luagit2.object_type(obj_tag)
		local type_commit = luagit2.object_type(obj_commit)

		assert.are.equal("blob",luagit2.object_type2string(type_blob))
		assert.are.equal("tree",luagit2.object_type2string(type_tree))
		assert.are.equal("tag",luagit2.object_type2string(type_tag))
		assert.are.equal("commit",luagit2.object_type2string(type_commit))
	end)

	it("Tests object short id",function()
		local short_id_buf = luagit2.object_short_id(obj_blob)
	...
	it("Tests object owner",function()
		...
		local owner_repo = luagit2.object_owner(obj_blob)
