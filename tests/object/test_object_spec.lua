...
describe(" Object Methods Tests ", function()
	...
	before_each(function()
		...
		blob_id = lib.luagit2_oid_fromstr(blob_id_str)
		tree_id = lib.luagit2_oid_fromstr(tree_id_str)
		tag_id = lib.luagit2_oid_fromstr(tag_id_str)
		commit_id = lib.luagit2_oid_fromstr(commit_id_str)
		otype_blob = lib.luagit2_object_string2type("blob")
		otype_tree = lib.luagit2_object_string2type("tree")
		otype_tag = lib.luagit2_object_string2type("tag")
		otype_commit = lib.luagit2_object_string2type("commit")
		obj_blob = lib.luagit2_object_lookup(repo,blob_id,otype_blob)
		obj_tree = lib.luagit2_object_lookup(repo,tree_id,otype_tree)
		obj_tag = lib.luagit2_object_lookup(repo,tag_id,otype_tag)
		obj_commit = lib.luagit2_object_lookup(repo,commit_id,otype_commit)
	end)

	after_each(function()
		lib.luagit2_object_free(obj_blob)
		lib.luagit2_object_free(obj_tree)
		lib.luagit2_object_free(obj_tag)
		lib.luagit2_object_free(obj_commit)
	...
	it("Tests getting object ID ",function()
		local check_blob_id = lib.luagit2_object_id(obj_blob)
		local check_blob_id_str = lib.luagit2_oid_tostr_s(check_blob_id)

		local check_tag_id = lib.luagit2_object_id(obj_tag)
		local check_tag_id_str = lib.luagit2_oid_tostr_s(check_tag_id)

		local check_tree_id = lib.luagit2_object_id(obj_tree)
		local check_tree_id_str = lib.luagit2_oid_tostr_s(check_tree_id)

		local check_commit_id = lib.luagit2_object_id(obj_commit)
		local check_commit_id_str = lib.luagit2_oid_tostr_s(check_commit_id)
	...
	it("Checks Object Type ",function()
		local type_blob = lib.luagit2_object_type(obj_blob)
		local type_tree = lib.luagit2_object_type(obj_tree)
		local type_tag = lib.luagit2_object_type(obj_tag)
		local type_commit = lib.luagit2_object_type(obj_commit)

		assert.are.equal("blob",lib.luagit2_object_type2string(type_blob))
		assert.are.equal("tree",lib.luagit2_object_type2string(type_tree))
		assert.are.equal("tag",lib.luagit2_object_type2string(type_tag))
		assert.are.equal("commit",lib.luagit2_object_type2string(type_commit))
	end)

	it("Tests object short id",function()
		local short_id_buf = lib.luagit2_object_short_id(obj_blob)
		local str_value = lib.luagit2_buf_details(short_id_buf)
		assert.is_not_nil(string.match(blob_id_str,str_value))
	end)

	it("Tests object owner",function()
		-- Get the owner repository
		local owner_repo = lib.luagit2_object_owner(obj_blob)
